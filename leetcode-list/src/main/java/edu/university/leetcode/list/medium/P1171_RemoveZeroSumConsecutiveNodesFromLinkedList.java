package edu.university.leetcode.list.medium;

import edu.university.leetcode.list.ListNode;

import java.util.HashMap;
import java.util.Map;

import static edu.university.leetcode.list.ListNode.*;

/**
 * 1171. Remove Zero Sum Consecutive Nodes from Linked List
 * Given the head of a linked list, we repeatedly delete consecutive sequences of nodes that sum to 0 until there are no such sequences.
 * After doing so, return the head of the final linked list.  You may return any such answer.
 * (Note that in the examples below, all sequences are serializations of ListNode objects.)
 * Example 1: Input: head = [1,2,-3,3,1] Output: [3,1]
 * Note: The answer [1,2,1] would also be accepted.
 * Example 2: Input: head = [1,2,3,-3,4] Output: [1,2,4]
 * Example 3: Input: head = [1,2,3,-3,-2] Output: [1]
 * Constraints:
 * The given linked list will contain between 1 and 1000 nodes.
 * Each node in the linked list has -1000 <= node.val <= 1000.
 */
public class P1171_RemoveZeroSumConsecutiveNodesFromLinkedList {
    public ListNode removeZeroSumSubLists(ListNode head) {
        Map<Integer, ListNode> map = new HashMap<>();
        int sum = 0;
        ListNode dummy = new ListNode(0);
        dummy.next = head;
        for (ListNode node = dummy; node != null; node = node.next) {
            sum += node.val;
            map.put(sum, node);
        }
        sum = 0;
        for (ListNode node = dummy; node != null; node = node.next) {
            sum += node.val;
            node.next = map.get(sum).next;
        }
        return dummy.next;
    }

    public static void main(String[] args) {
        P1171_RemoveZeroSumConsecutiveNodesFromLinkedList test = new P1171_RemoveZeroSumConsecutiveNodesFromLinkedList();
        ListNode test1 = new ListNode(1);
        insertEnd(test1, 2);
        insertEnd(test1, 3);
        insertEnd(test1, -3);
        insertEnd(test1, 1);
        System.out.println(printNode(test.removeZeroSumSubLists(test1)));
        ListNode test2 = new ListNode(1);
        insertEnd(test2, 2);
        insertEnd(test2, 3);
        insertEnd(test2, -3);
        insertEnd(test2, -2);
        System.out.println(printNode(test.removeZeroSumSubLists(test2)));
    }
}
